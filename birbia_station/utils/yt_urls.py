class YtUrls:
    ...
    def url_corrector(self, url: str):
        """
        Corrects any URLs passed in to find the correct audio.
        """

        if len(url.split("?v=")) >= 2:
            return self.YT_BASE_URL + url.split("?v=")[1][:11]
        if len(url.split("/v/")) >= 2:
            return self.YT_BASE_URL + url.split("/v/")[1][:11]
        if len(url.split("/youtu.be/")) >= 2:
            return self.YT_BASE_URL + url.split("/youtu.be/")[1][:11]
        if len(url.split("/shorts/")) >= 2:
            return self.YT_BASE_URL + url.split("/shorts/")[1][:11]
